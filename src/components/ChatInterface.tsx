...
export default function ChatInterface() {
  ...
  const runInSandbox = async (code: string, lang: string) => {
    ...
    setSandboxOutput({ code, output: 'INITIALIZING_SANDBOX...', type: normalizedLang === 'python' ? 'python' : normalizedLang === 'js' || normalizedLang === 'javascript' ? 'js' : 'error' });
    ...
    if (normalizedLang === 'javascript' || normalizedLang === 'js') {
      ...
      setTimeout(() => {
        setSandboxOutput(prev => prev ? { 
          ...
          `[SUCCESS] Python script execution completed. Data piped to Nightfury Engine.`
        } : null);
      }, 2000);
    } else if (normalizedLang === 'bash' || normalizedLang === 'sh' || normalizedLang === 'shell') {
      setSandboxOutput({ code, output: 'INITIALIZING_VIRTUAL_SHELL...\nLOADING_RECON_TOOLS...\nSETTING_UP_ENV...', type: 'js' });
      
      setTimeout(() => {
        setSandboxOutput(prev => prev ? { 
          ...prev, 
          output: prev.output + `\n\n[SHELL_OPS_LOG]\nTARGET: ${targetDomain}\nCOMMAND: EXECUTING_BASH_SCRIPT\n\n` + 
          `$ nmap -sV ${targetDomain} --script vuln\n` +
          `$ gobuster dir -u https://${targetDomain} -w /usr/share/wordlists/dirb/common.txt\n\n` +
          `[SUCCESS] Bash script execution completed. Logs captured.`
        } : null);
      }, 1500);
    } else {
